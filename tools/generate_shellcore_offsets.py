...
# Firmware 2.00 only has a 0x310-byte executable tail cave.  Kstuff's PPR
# bridge owns the first 0x300 bytes, leaving no safe room for ShadowMount's
# bridge.  Keep this exclusion in the generator so regenerating the table
# cannot silently reintroduce the collision.
UNSUPPORTED_BRIDGE_FIRMWARES = {
    0x0200: "executable cave overlaps the kstuff PPR bridge",
}
...
def generate(root: Path) -> str:
    ...
    for firmware_dir in firmware_dirs:
        ...
        if firmware in UNSUPPORTED_BRIDGE_FIRMWARES:
            continue
    ...
    if not records:
        raise ValueError(f"{root}: no supported SceShellCore firmware files found")
